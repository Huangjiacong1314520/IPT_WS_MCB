...
int   ReadRegCommon(Uint32 uRegAddress, Uint8 cRegLen, Uint16  uChannelSel, Uint8  cTransactionType, Uint8  uChannel, /*Uint32  uWriteRequestAddr, */ AuroraPacket* pPkt)
{
   ...
   //            unsigned int uSlotNum = 2;
        ...
        MyMemSet(_buffer,  0,  sizeof _buffer);  //g_localAuroraPktBuffer

        Uint32 uReadAddr =  uChannel*0x10000 + 0x80100000;
        uReadAddr += uRegAddress*0x100;
        int  nNReadRet = SRIOReadData(0xff,  uReadAddr,  SRIO_PACKET_TYPE_NRead , _buffer,  sizeof _buffer);
        if (0 != nNReadRet)
                 return -1;
 ...
 //                    Invalidate_Cache(1, (void*)g_localAuroraPktBuffer ,  sizeof g_localAuroraPktBuffer, 1);
                     ...
                     int nCount = 0;
                     volatile Uint32* pData = (Uint32*) (_buffer  + 4);
                        while( 0 == (*pData  ))
                        {
                            nCount++;
                            if  (nCount >= 0xFFF)
                 ...
                 pPkt->head     = (AuroraPacketHead*)_buffer ;
                 for (i = 0;  i <= pPkt->head->packet_len;  i++)
                 {
                     pPkt->body[i] = (AuroraPacketBody*)(_buffer + sizeof(AuroraPacketHead) + sizeof(AuroraPacketBody)*i);
