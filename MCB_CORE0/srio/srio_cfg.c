...
int  ReadSYSSlotNumReg(Uint32*  uNO)
{
    unsigned char cFPGAID = FPGA_DEVICE_ID_8BIT;
        Uint32   uRdAddr = SRIO_SYS_REG_BASE + SRIO_SYS_SLOT_NUM_REG;
        unsigned char cPackType = SRIO_PACKET_TYPE_NRead;
        unsigned char pLocalBuff[8] = {0};
        int nLen = 8;
...
{
        unsigned char cFPGAID = FPGA_DEVICE_ID_8BIT;
        Uint32   uWrAddr = SRIO_SYS_REG_BASE + SRIO_SYS_FRONT_LED_REG;
        unsigned char cPackType = SRIO_PACKET_TYPE_NWRITE;
        unsigned char pLocalBuffer[8] ={0};
        int nLen = 8;
...
{
    unsigned char cFPGAID = FPGA_DEVICE_ID_8BIT;
           Uint32   uRdAddr = SRIO_SYS_REG_BASE + SRIO_SYS_SFP_STA_REG;
           unsigned char cPackType = SRIO_PACKET_TYPE_NRead;
           unsigned char pLocalBuff[8] = {0};
           int nLen = 8;
...
{
    unsigned char cFPGAID = FPGA_DEVICE_ID_8BIT;
    Uint32   uWrAddr = SRIO_SYS_REG_BASE + SRIO_SYS_P2BUS_MODE_REG;
    unsigned char cPackType = SRIO_PACKET_TYPE_NWRITE;
    unsigned char pLocalBuffer[8] ={0};
    int nLen = 8;
...
{
        unsigned char cFPGAID = FPGA_DEVICE_ID_8BIT;
        Uint32   uWrAddr = SRIO_SYS_REG_BASE + SRIO_SYS_GPIO_SEL_REG;
        unsigned char cPackType = SRIO_PACKET_TYPE_NWRITE;
        unsigned char pLocalBuffer[8] ={0};
        int nLen = 8;
...
{
      unsigned char cFPGAID = FPGA_DEVICE_ID_8BIT;
       Uint32   uRdAddr = SRIO_SYS_REG_BASE + SRIO_SYS_GPI_REG;
       unsigned char cPackType = SRIO_PACKET_TYPE_NRead;
       unsigned char pLocalBuff[8] = {0};
       int nLen = 8;
       SRIORWStatus  status = SRIOReadData( cFPGAID, uRdAddr, cPackType ,   pLocalBuff,   nLen);
...
{
    unsigned char cFPGAID = FPGA_DEVICE_ID_8BIT;
    Uint32   uWrAddr = SRIO_SYS_REG_BASE + SRIO_SYS_GPO_REG;
    unsigned char cPackType = SRIO_PACKET_TYPE_NWRITE;
    unsigned char pLocalBuffer[8] ={0};
    int nLen = 8;
...
#endif
Uint8  g_localAuroraPktBuffer[512] = {0};
...
#endif
Uint8  g_localAuroraPktBufferIndex[PacketBodyMax] = {0};
...
{
    Uint32 uRegAddress =  SRIO_DA_SPI_BAUD;
    Uint8 cRegLen  = 3;
    return ReadADReg( uRegAddress,  cRegLen,   uChannel,    cTransactionType,    uReadAddr,   uWriteRequestAddr, pPkt );
}
...
{
        Uint32 uRegAddress =  SRIO_DA_TEST_EN;
        Uint8 cRegLen  = 3;
        return ReadADReg( uRegAddress,  cRegLen,   uChannel,    cTransactionType,    uReadAddr,   uWriteRequestAddr, pPkt );
}
...
{
        Uint32 uRegAddress =  SRIO_DA_WAVE_SEL;
        Uint8 cRegLen  = 3;
        return ReadADReg( uRegAddress,  cRegLen,   uChannel,    cTransactionType,    uReadAddr,   uWriteRequestAddr, pPkt );
}
...
{
    Uint32 uRegAddress =  SRIO_DA_PERIOD;
    Uint8 cRegLen  = 3;
    return ReadADReg( uRegAddress,  cRegLen,   uChannel,    cTransactionType,    uReadAddr,   uWriteRequestAddr, pPkt );
}
...
{
    Uint32 uRegAddress =  SRIO_DA_FREQ;
    Uint8 cRegLen  = 3;
    return ReadADReg( uRegAddress,  cRegLen,   uChannel,    cTransactionType,    uReadAddr,   uWriteRequestAddr, pPkt );
}
...
{
    Uint32 uRegAddress =  SRIO_DA_AMP;
    Uint8 cRegLen  = 3;
    return ReadADReg( uRegAddress,  cRegLen,   uChannel,    cTransactionType,    uReadAddr,   uWriteRequestAddr, pPkt );
}
...
//计算电涡流传感器位移:
inline  double  GetEddyCurrentSensorOffset(int num)
{
    double tmp = 2.0*num;
    tmp /= (0x1000000-1);
    tmp = tmp -1;
...
//绝对式光栅尺位移
inline  double GetAbsoluteLinearEncoderOffset(int num)
{
    return num*1.0/1000000000;
}
//增量式光栅尺位移
inline  double GetDILAOffset(int num)
{
    return   num*25.0/1000000000;
}
