#include <stdio.h>
#include "../drv_pll.h"
#include "../include/hw_C6678.h"


#if 1
#define GEL_VERSION     2.005
#define DNUM            0
 ...
 */
Delay_milli_seconds(int myDelay)
{
    myDelay *= 100000;
    while(myDelay--);
 ...
 *
 ****************************************************************************/
int prog_pll1_values(unsigned int pll_multiplier, unsigned int pll_divider, unsigned int odiv)
{
    int TEMP;
...
Set_PSC_State(unsigned int pd,unsigned int id,unsigned int state)
{
    ...
    // logPrintf( "Set_PSC_State for pd = %d \n",,2,,,pd);
...
{
    int iResult,iTemp;
    double fTemp, fSTolP, fPar, fZero;
...
void Set_DSP_Cache( )
{
    ...
    CACHE_L1PCFG = 4;           // L1P on, MAX size,32K
    CACHE_L1DCFG = 4;           // L1D on, MAX size,32K
    CACHE_L2CFG  = 0;           // L2 off, use as RAM,0
...
void Setup_Memory_Map( )
{
    ...
    logPrintf( "Setup_Memory_Map... Done.\n" );
}
#endif
/****************************************************************************
 *
...
Global_Default_Setup_Silent()
{
    ...
    unsigned int PLLM_DDR = 39;
    unsigned int PLLD_DDR = 2;
...
void DDR3_Setup_1333()
{
	...
	DDR3_CONFIG_REG_23 |= 0x00000200;    //Set bit 9 = 1 to use forced ratio leveling for read DQS
										 ...
										 //Values with invertclkout = 1
										 /**************** 3.3 Partial Automatic Leveling ********************/
	DATA0_WRLVL_INIT_RATIO = 0x94;
	DATA1_WRLVL_INIT_RATIO = 0x8C;
	DATA2_WRLVL_INIT_RATIO = 0x9B;
	DATA3_WRLVL_INIT_RATIO = 0x8F;
	DATA4_WRLVL_INIT_RATIO = 0xA2;
	DATA5_WRLVL_INIT_RATIO = 0x92;
	DATA6_WRLVL_INIT_RATIO = 0x9A;
	DATA7_WRLVL_INIT_RATIO = 0x97;
	DATA8_WRLVL_INIT_RATIO = 0x9A;

	DATA0_GTLVL_INIT_RATIO = 0xD4;
	DATA1_GTLVL_INIT_RATIO = 0xDD;
	DATA2_GTLVL_INIT_RATIO = 0xCD;
	DATA3_GTLVL_INIT_RATIO = 0xD9;
	DATA4_GTLVL_INIT_RATIO = 0xC7;
	DATA5_GTLVL_INIT_RATIO = 0xD7;
	DATA6_GTLVL_INIT_RATIO = 0xCF;
	DATA7_GTLVL_INIT_RATIO = 0xD1;
	DATA8_GTLVL_INIT_RATIO = 0xCF;
...
DDR3_Setup_1600()
{
	...
	DDR_SDRFC = 0x00005162;    // enable configuration
							   ...
							   /* DDR_SDTIM1   = 0x1113783C; */
	DDR_SDTIM1 = 0x1557CA05;

	/* DDR_SDTIM2   = 0x30717FE3; */
	DDR_SDTIM2 = 0x40877FEC;

	/*  DDR_SDTIM3   = 0x559F86AF; */
	DDR_SDTIM3 = 0x55BF87FF;
	...
	DDR_SDRFC = 0x000061A7; // enable configuration
	...
	DDR_SDRFC = 0x00001869;       //Refresh rate = (7.8*666MHz)
...
void Init_DDR3_Timing(DDR3Timing Timing)
{
	...
	switch(Timing)
	{
		case DDR3_TIMING_1333MHZ:
			DDR3_Setup_1333();
			break;
		case DDR3_TIMING_1600MHZ:
			DDR3_Setup_1600();
			break;
	}
}
