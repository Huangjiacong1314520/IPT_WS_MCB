...
int ReadData(char *filename,double *storge,unsigned int max_len)
{
    int i;
    FILE *fp = fopen(filename,"r");
    if (!fp){
        printf("open file error file name is %s\n",filename);
        return -1;
    }
    for(i = 0;i < max_len; ++i){
        if (fscanf(fp, "%lf\n",&storge[i]) == EOF) {
        ...
        storge[i] = storge[i] * 65535.5 *0.1;
    }
    fclose(fp);
    return 0;
}
int WriteData(char *filename,double *storge,unsigned int max_len)
{
    int i;
...
void MovePhaseCheck(int index, Axis axis, unsigned long long count, double Num , unsigned long long int topdelay)
{

    if (index >= 1 && index < 8){
        MultiCoreShareData->Phase[axis] = Acc;
    }else if (index >= 8 && index < 9){
        MultiCoreShareData->Phase[axis] = Uni;
    }else if (index >= 9 && index < 16){
        MultiCoreShareData->Phase[axis] = Dec;
    }else if (index == 16){
        MultiCoreShareData->Phase[axis] = Loc;
    }else{
        ;
    }
    if (index == 16 && (count < Num + topdelay) && (count >= Num - 1)){  //count >= Num - 1存疑
        MultiCoreShareData->Phase[axis] = Dec;
    }
}
